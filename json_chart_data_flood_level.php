...
if ($_GET['mode'] == 'latest') {
	if (isset($_GET['sensor_id'])) {
		...
		$sensor_id = $_GET['sensor_id'];
		$query = $pdo->query("SELECT * FROM (SELECT *  FROM `sensor_logs` WHERE `sensor_id` = '$sensor_id' AND is_active = 1 ORDER BY timestamps DESC LIMIT 30) as tbltemp ORDER BY timestamps ASC");
	...
	$sql_tmp_fd_hr = "SELECT DISTINCT HOUR(sensor_log_tmp.timestamps) as hours, DATE(sensor_log_tmp.timestamps) as date_tmp  FROM (SELECT * FROM sensor_logs WHERE sensor_id = '$sensor_id'  AND is_active = 1 ) AS sensor_log_tmp WHERE DATE(sensor_log_tmp.timestamps) = CURDATE();";
	...
	// hours disp
		...
		$sql_tmp_fd_avg = "SELECT AVG(sensor_log_tmp.sensor_value) as avg_tmp FROM (SELECT * FROM sensor_logs WHERE sensor_id = '$sensor_id'  AND is_active = 1 ) AS sensor_log_tmp WHERE DATE(sensor_log_tmp.timestamps) = '$date_obj_tmp' AND HOUR(sensor_log_tmp.timestamps) = '$hours_obj_tmp'";
	...
	$sql_tmp_fd_dly = "SELECT DISTINCT DATE(sensor_log_tmp.timestamps) as date_tmp, WEEK(sensor_log_tmp.timestamps) as week_tmp FROM (SELECT * FROM sensor_logs WHERE sensor_id = '$sensor_id'  AND is_active = 1) AS sensor_log_tmp WHERE WEEK(sensor_log_tmp.timestamps) = WEEK(CURDATE());";
	...
	$sql_tmp_fd_dly = "SELECT DISTINCT DATE(sensor_log_tmp.timestamps) as date_tmp, MONTH(sensor_log_tmp.timestamps) as month_tmp FROM (SELECT * FROM sensor_logs WHERE sensor_id = '$sensor_id'  AND is_active = 1 ) AS sensor_log_tmp WHERE MONTH(sensor_log_tmp.timestamps) = MONTH(CURDATE());";
	...
	// hours disp
		...
		$sql_tmp_fd_mt_avg = "SELECT AVG(sensor_log_tmp.sensor_value) as avg_tmp FROM (SELECT * FROM sensor_logs WHERE sensor_id = '$sensor_id'  AND is_active = 1 ) AS sensor_log_tmp WHERE MONTH(sensor_log_tmp.timestamps) = MONTH(CURDATE()) AND DATE(sensor_log_tmp.timestamps) = DATE('$date_obj_tmp')";
